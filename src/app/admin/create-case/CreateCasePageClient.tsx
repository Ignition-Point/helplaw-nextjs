...
export default function CreateCasePageClient() {
                    ...
                    <>
                      ...
                      <div className="text-navy-900">
                        ...
                        {result.imageOptimization.originalDimensions && (
                          <span className="text-slate-400 ml-1">
                            ({result.imageOptimization.originalDimensions.width}x{result.imageOptimization.originalDimensions.height})
                          </span>
                        )}
                      </div>
                      ...
                      <div className="text-green-700 font-medium">
                        ...
                        {result.imageOptimization.finalDimensions && (
                          <span className="text-slate-400 ml-1">
                            ({result.imageOptimization.finalDimensions.width}x{result.imageOptimization.finalDimensions.height})
                          </span>
                        )}
                      </div>
                  ...
                  {result.qcWarnings
                    ...
                    .map((w: { field: string; severity: string; message: string }, i: number) => (
                      <li key={i} className="flex items-start gap-1.5">
                        <span className="mt-0.5 shrink-0 w-2 h-2 rounded-full bg-red-500" />
                        <span>
              ...
              <div className="mt-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                ...
                <ul className="text-xs text-amber-700 space-y-1">
                  {result.qcWarnings
                    ...
                    .map((w: { field: string; severity: string; message: string }, i: number) => (
                      <li key={i} className="flex items-start gap-1.5">
                        <span className="mt-0.5 shrink-0 w-2 h-2 rounded-full bg-amber-400" />
                        <span>
                          <strong className="font-medium">{w.field}:</strong> {w.message}
                        </span>
                      </li>
